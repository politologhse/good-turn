...
VK ограничивает ~5 Мбит/с на TURN-подключение. GUI по умолчанию стартует в balanced-режиме `x2`, чтобы получить примерно вдвое больше пропускной способности без ручной настройки. Если нужно больше, увеличьте `Streams` в Settings; `x4` обычно даёт около ~20 Мбит/с.
